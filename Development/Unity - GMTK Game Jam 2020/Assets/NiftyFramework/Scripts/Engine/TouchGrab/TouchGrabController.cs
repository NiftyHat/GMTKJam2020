...
namespace NiftyFramework.TouchGrab
{
    public class TouchGrabController : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            inputController.OnPrimaryInputStart += OnPrimaryInputStart;
            inputController.OnPrimaryInputMoved += OnPrimaryInputUpdate;
            //inputController.OnInputStationary += OnInputUpdate;
            inputController.OnPrimaryInputEnd += OnPrimaryInputEnd;
        ...
        private void OnPrimaryInputStart(Vector2 position, Vector2 delta, Ray screenPointRay, float time, int inputId)
        {
            _heldItemList.Clear();
            RaycastHit hitInfo;
        ...
        private void OnPrimaryInputUpdate(Vector2 position, Vector2 delta, Ray screenPointRay, float time, int inputId)
        {
            RaycastHit hitInfo;
            bool isHit = Physics.Raycast(screenPointRay, out hitInfo, maxDistance);
        ...
        private void OnPrimaryInputEnd(Vector2 position, Vector2 delta, Ray screenPointRay, float time, int inputId)
        {
